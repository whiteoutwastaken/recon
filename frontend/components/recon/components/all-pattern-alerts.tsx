...
export function AllPatternAlerts({ competitors }: { competitors: Competitor[] }) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 delay-200">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-base font-semibold text-foreground">All Pattern Alerts</h3>
          <p className="text-sm text-muted-foreground mt-0.5">AI-detected patterns across all competitors</p>
        </div>
        <span className="text-xs text-muted-foreground">{patterns.length} active patterns</span>
      </div>

      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-48 bg-secondary/30 rounded-xl animate-pulse" />)}
        </div>
      ) : patterns.length === 0 ? (
        <div className="text-muted-foreground text-sm py-8 text-center">No patterns detected yet. Click "Detect patterns" in the sidebar.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {patterns.map((pattern, index) => {
            ...
            return (
              <div key={pattern.pattern_id} className="bg-card border border-border rounded-xl p-5 hover:border-foreground/30 transition-all duration-300 animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${index * 80}ms`, animationFillMode: "both" }}>
                ...
                <button onClick={() => handleAsk(pattern)} disabled={asking === pattern.pattern_id} className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-medium bg-secondary text-foreground hover:bg-secondary/80 transition-all duration-200 disabled:opacity-50">
